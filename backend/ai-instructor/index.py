...
def handler(event: dict, context) -> dict:
    ...
    if student_name:
        full_prompt += f'\n\nИмя ученика, с которым ты сейчас разговариваешь: {student_name}. Ты знаешь его имя, но НЕ вставляй его в каждое сообщение — в реальной жизни люди не называют собеседника по имени постоянно. Используй имя редко и уместно: например, при первом приветствии, когда хочешь подчеркнуть важный момент, подбодрить или обратить особое внимание. В большинстве обычных реплик обходись вообще без имени.'
    if forbidden.strip():
